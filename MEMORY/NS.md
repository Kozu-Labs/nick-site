...
### 2026-07-13 — IP enforcement search and authority expansion
- Expanded the authority site from six launch pages to fourteen indexed public routes. Added distinct patent enforcement, trademark infringement, copyright infringement, Amazon IP enforcement, Insights, and three practice-note pages while retaining the attorney-first trust sequence and keeping CopyCatch outside Nick's first viewport.
- The content architecture now serves two primary inbound audiences: IP owners seeking help with an active infringement problem and attorneys/law firms evaluating Schedule A or marketplace-scale enforcement. Each right has a separate thesis and decision framework; the pages are not keyword-swapped duplicates.
- Fable's dedicated CC critique informed the supervision boundary, owner-routing split, conflicts-safe intake, Amazon three-way decision tool, and `Schedule A is not automatic` framing. Growth independently retained copyright as a full search surface because it is part of the intended service expansion, while declining unsupported bylines, performance claims, results claims, and software-first framing.
- Competitive/search findings and the writing standard are documented in `docs/marketing/ip-enforcement-competitive-search-analysis-2026-07-13.md`. The strategy covers patent, trademark, copyright, Amazon enforcement, Schedule A, attorney-referral, and AI-citation discovery intent.
- Public-source release commit `bd97a65` was rebased and pushed before Firebase Hosting deployment. Both `https://nslegal-ip.com` and `https://nick-site-web.web.app` returned `200` for all fourteen public routes. Desktop and 390px mobile QA passed without horizontal overflow; mobile navigation, CTA sizing, canonical tags, JSON-LD, sitemap, internal links, and structured article layouts passed. Protected portal noindex headers remain intact.
- The existing contact-to-Slack function and Firebase rewrites were not changed. The browser now carries a non-sensitive `regarding` source field into the existing intake payload so inquiries can be attributed to patent, trademark, copyright, Amazon, referral, or article entry points. No production form submission was generated for this release.
- Nick/legal approval remains required before active promotion, especially for attorney-advertising compliance, visible credentials, and legal explanatory content. Clio-derived examples, outcomes, representative matters, and results remain excluded pending a minimum-data protocol and explicit approval.
